...
- CoinDesk's currently supported product is the `Data API` at `https://data-api.coindesk.com`.
...
- The docs describe the API as JSON-first, while also noting that most REST endpoints can return `CSV` when the route supports `response_format=CSV`.
- The reviewed docs repeatedly show the common response envelope shape as top-level `Data` and `Err`, and the introduction also documents a `Warn` field for partial-but-usable responses.

## Authentication

### REST authentication methods confirmed from the Introduction page
...
- `https://data-api.coindesk.com/index/cc/v1/latest/tick?market=ccix&instruments=BTC-USD&api_key=YOUR_API_KEY`
...
### WebSocket authentication confirmed from the Introduction page
CoinDesk documents two WebSocket auth patterns:
- URL parameter auth: `wss://data-streamer.coindesk.com/?api_key=YOUR_API_KEY`
...
## Rate limits
- The REST introduction says rate limiting is based on **calls, not credits**.
...
- The introduction states that API responses are provided in `JSON`.
- The introduction also says most endpoints support `CSV` output where route-specific `response_format` support is documented.
- The reviewed REST route pages consistently document a top-level response envelope with `Data` and `Err` objects.
- The REST introduction also documents a `Warn` field for ambiguous or partial-but-satisfactory responses.
...
- I did **not** find one universal `page`/`offset` pagination contract in the reviewed CoinDesk docs.
...
- fireROUTE normalization note: treat CoinDesk pagination/windowing as **endpoint-specific**, not a single shared API-wide contract.
...
## Official route inventory confirmed from the docs sidebar
The official CoinDesk docs sidebar currently exposes `93` concrete routes/streams.
...
- Use the `Data API` docs host, not the obsolete `old.coindesk.com/coindesk-api/` page.
- Treat CoinDesk as a broad market-data platform, not a small single-purpose ticker API: the official docs cover indices, spot, futures, options, on-chain DEX, chain data, asset metadata, news, overview aggregates, and WebSocket streaming.
- Many routes accept `groups` arrays to trim or expand the response schema; this is a recurring cross-product pattern in the docs.
...
## Verification notes
- All facts above came from manual review of the official CoinDesk developer documentation using browser tools.
- The `93` route/stream count is grounded in the visible official docs sidebar plus direct review of representative route pages.
- This file intentionally avoids filling gaps from third-party mirrors or old CryptoCompare-era summaries beyond what the reviewed official CoinDesk docs explicitly state.