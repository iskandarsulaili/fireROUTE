...
- Provider slug: `transport-for-bordeaux-france`
- Official docs used manually:
  - `https://opendata.bordeaux-metropole.fr/explore/`
  - `https://opendata.bordeaux-metropole.fr/api/explore/v2.1/swagger.json`
...
- Primary response / payload formats:
  ...
  - file downloads for export routes, including CSV and the other formats listed by the official export endpoints
- Transport scope documented here: the official Bordeaux Métropole Opendatasoft Explore API that powers transport / mobility dataset access on the public portal
...
- The official portal page shows `Transports/mobilité` as a dataset theme on the site and displayed `129` transport / mobility datasets at inspection time.
- The official docs state that all endpoints use Opendatasoft Query Language (`ODSQL`), so query features such as `select`, `where`, `order_by`, `group_by`, `refine`, and `exclude` are shared across many routes.
- The official docs state that `records` queries are limited, while `exports` are the route family to use when larger extracts are needed.
- The portal also exposes other geospatial tooling such as cartographic and OGC pages, but the route-level REST reference currently exposed for manual verification is the Explore API documented here.

## Rate limits, pagination, and errors
- A live unauthenticated request to `GET https://opendata.bordeaux-metropole.fr/api/explore/v2.1/catalog/datasets?limit=1` returned these rate-limit headers:
...
### Catalog routes
1. `GET /catalog/datasets`
- Purpose: query datasets published in the Bordeaux Métropole portal catalog
...
- `https://opendata.bordeaux-metropole.fr/explore/`
- `https://opendata.bordeaux-metropole.fr/api/explore/v2.1/swagger.json`
- `https://opendata.bordeaux-metropole.fr/api/explore/v2.1/catalog/datasets?limit=1`
